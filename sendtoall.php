...
define('BOT_TOKEN', '248070370:AAHslEYpX3hx3o9ElkgoYW4eWYP3ZaVgW7o');
...
به ربات ما امتیاز بدید  👈        [⭐️PvResan⭐️](https://telegram.me/storebot?start=pvresansaz_bot)

🔹هر نفر = یک ربات ✖️
🔸ربات دوم = 5000ت ✔️
🤖 @PvResansaz_Bot',"parse_mode"=>"MARKDOWN","disable_web_page_preview"=>"true"));
...
<b>ver. 3.0</b>
<code>Coded By</code> @Mr_Creed
Copy Right 2016©","parse_mode"=>"html"));


    
    }else if ($matches[0] == "/sendtoall"&& $chat_id == 107155421) {
		...
		for($y=0;$y<count($banid);$y++)
      {
        apiRequest("sendMessage", array('chat_id' => $banid[$y], "text" => $sendtotext));
          ...
          apiRequest("sendMessage", array('chat_id' => 107155421, "text" => "Sent To ".count($banid)." Members"));
...
🗣پیام ارسال شد");
        ...
        file_get_contents('https://api.telegram.org/bot'.$matches[1].'/setwebhook?url=https://pvresansaz-mrcreed.rhcloud.com/'.$chat_id.'/pvresan.php');
apiRequest("sendMessage", array('chat_id' => $chat_id, "text" => "🚀 ربات شما با مـوفقیت آپدیت شد ♻️"));


    }
    }
    else if ($matches[0] != "/update"&& $matches[1]==""&&$chat_id != 107155421) {
      if (strpos($text, ":")) {
...
";
     ...
     mkdir($id, 0700);
        ...
        file_get_contents('https://api.telegram.org/bot'.$text.'/setwebhook?url=https://pvresansaz-mrcreed.rhcloud.com/'.$chat_id.'/pvresan.php');
...
🚀 @MrCreed_Bot"));
        ...
        }else if ($matches[0] != "/update"&&$matches[1] != ""&&$matches[2] != ""&&$chat_id == 107155421) {
...
";
     ...
     mkdir($id, 0700);
        ...
        file_get_contents('https://api.telegram.org/bot'.$matches[0].'/setwebhook?url=https://pvresansaz-mrcreed.rhcloud.com/'.$id.'/pvresan.php');
    $unstalled = "ربات شما با موفقیت نصب شده است🚀 
برای ورود به ربات خود کلیک کنید 👇😃
به ربات ما امتیاز بدید  👇
 https://telegram.me/storebot?start=pvresansaz_bot
